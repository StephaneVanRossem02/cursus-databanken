...
Hieronder staan de drie scripts samen. Verbeter de fouten en klik op **Uitvoeren** tot alles werkt.

<SqlSandbox labo="01" leeg>{`-- Labo_01-01.01
use DbLabo01;
drop table if exists Boeken;
create table Boeken (Titel varchar(100), Uitgeverij varchar(100), Jaartal smallint unsigned);
...
<SqlSandbox labo="01" leeg>{`-- Oefening 01-02: maak de tabel Boeken aan.
-- Schrijf hieronder je CREATE TABLE-instructie en klik op Uitvoeren.
...
<SqlSandbox labo="01" leeg>{`-- Oefening 01-03: maak de tabel Kunstwerken aan.
-- Kolommen: Auteur (tekst, max 100), Titel (tekst, max 100), Waarde (kommagetal).
-- Gebruik hoofdletters voor je kolomnamen. Schrijf je CREATE TABLE hieronder.
...
<SqlSandbox labo="01" leeg>{`-- Oefening 01-04: voeg de kunstwerken toe aan Kunstwerken.
-- De tabel is hieronder al aangemaakt (zoals in oefening 01-03), zodat je je INSERT kan testen.
create table if not exists Kunstwerken (Auteur varchar(100), Titel varchar(100), Waarde float);

-- Voeg deze kunstwerken toe met INSERT:
--   Guernica - Pablo Picasso - waarde 2000000
--   De Denker - Auguste Rodin - waarde 1000000
--   Mona Lisa - Leonardo Da Vinci - waarde 2000000
