...
/** Zet een fout om in een begrijpelijke boodschap. */
      ...
      {!key || toontKeyForm ? (
        ...
        <>
          {berichten.length === 0 && (
            <>
              <p className={styles.uitleg}>
                Snap je de opgave niet goed? Vraag om ze anders uit te leggen. Loopt je query
                vast? Beschrijf wat er misgaat, en stuur je SQL mee als je wil. Je krijgt nooit
                de oplossing, wel iets waarmee je zelf verder geraakt.
              </p>
              <div className={styles.voorzetten}>
                {[
                  'Leg deze opgave eens op een andere manier uit.',
                  'Wat moet ik hier precies opvragen of aanmaken?',
                  'Ik weet niet hoe ik aan deze query begin.',
                  'Ik krijg een foutmelding en snap niet waarom.',
                ].map((tekst) => (
                  <button
                    key={tekst}
                    type="button"
          ...
          <form onSubmit={verstuur} className={styles.vraagForm}>
            <textarea
              ...
              placeholder="Waar loop je vast? Bijvoorbeeld: mijn WHERE toont te veel rijen."
              rows={2}
