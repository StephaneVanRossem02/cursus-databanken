...
Hoe je de tweets terug koppelt aan de juiste account, lees je ook bij de uitleg rond [`JOIN`-operaties bij simpele relaties](/docs/my-sql/joins-basic/joins-simpele-relaties). We tonen hier alleen dat het mogelijk is. Koppel users aan de juiste tweets met dit script, 0058\_\_SelectUsersTweets.sql:
...
[`JOIN`-operaties bij simpele relaties](/docs/my-sql/joins-basic/joins-via-tussenliggende-tabel) legt uit hoe je nu toont welke games op welk platform verschenen zijn. Dit wordt hier gedaan in 0061\_\_SelectReleases.sql:
