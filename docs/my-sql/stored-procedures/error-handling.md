...
![](/img/databanken/sp_errorhandling1.JPG)
...
![](/img/databanken/sp_errorhandling2.JPG)
...
![](/img/databanken/sp_errorhandling3.JPG)

Je merkt op dat de derde handler werd uitgevoerd en dit omdat de error `1062` exact aansluit bij de exception. In de meeste programmeertalen is dit anders en wordt de *eerste* handler (bv. een `catch`-blok in C# of een `except`-blok in Python) gebruikt. In MySQL is het **de meest specifieke** handler.
